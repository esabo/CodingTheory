...
function StabilizerCodeCSS(
    ...
    C2 ⊆ C1 || throw(
        ArgumentError(
            "The second argument must be a subset of the first in the CSS construction.",
        ),
    )
    ...
    if !iszero(stand_k)
        if logs_alg == :stnd_frm
            logs = _make_pairs(
                _logicals_standard_form(stabs_stand, C1.n, stand_k, stand_r, P_stand),
            )
    ...
    signs, X_signs, Z_signs =
        _determine_signs_CSS(stabs, char_vec, nrows(D2.H), nrows(C1.H))
    ...
    if !iszero(stand_k)
        ...
        u_bound = min(u_bound_dx, u_bound_dz)
        if !ismissing(C1.d) && is
            !ismissing(D2.d)
            u_bound_classical = min(C1.d, D2.d)
            u_bound_classical < u_bound && (u_bound = u_bound_classical;)
        end
        # TODO set upper bounds to distance if known
        return GraphStateStabilizerCSS(
            C1.F,
            C1.n,
            0,
            missing,
...
CSSCode(
    ...
    char_vec::Union{Vector{zzModRingElem},Missing} = missing,
    logs_alg::Symbol = :stnd_frm,
) = StabilizerCodeCSS(C1, C2, char_vec = char_vec, logs_alg = logs_alg)
...
function StabilizerCodeCSS(
    ...
    if !iszero(stand_k)
        if logs_alg == :stnd_frm
            logs = _make_pairs(
                _logicals_standard_form(stabs_stand, C.n, stand_k, stand_r, P_stand),
            )
            logs_mat = reduce(vcat, [reduce(vcat, logs[i]) for i = 1:length(logs)])
        else
...
CSSCode(
    ...
    char_vec::Union{Vector{zzModRingElem},Missing} = missing,
    logs_alg::Symbol = :stnd_frm,
) = StabilizerCodeCSS(C, char_vec = char_vec, logs_alg = logs_alg)
...
function StabilizerCodeCSS(
    ...
    n = ncols(X_matrix)
    n == ncols(Z_matrix) || throw(
        ArgumentError("Both matrices must have the same length in the CSS construction."),
    )
    F = base_ring(X_matrix)
    F == base_ring(Z_matrix) || throw(
        ArgumentError(
            "Both matrices must be over the same base field in the CSS construction.",
        ),
    )
    iszero(Z_matrix * transpose(X_matrix)) ||
    ...
    nrows(X_matrix) > X_rank || nrows(Z_matrix) > Z_rank ? (over_comp = true) :
    (over_comp = false)
    ...
    if !iszero(stand_k)
        if logs_alg == :stnd_frm
            ...
            # n + (n - Srank)
            nrows(H) == 2 * n - X_rank - Z_rank ||
                error("Normalizer matrix is not size n + k.")
            logs, logs_mat = _logicals(stabs, H_tr, logs_alg)
        end
    end
    signs, X_signs, Z_signs =
        _determine_signs_CSS(stabs, char_vec, nrows(X_matrix), nrows(Z_matrix))
...
CSSCode(
    ...
    StabilizerCodeCSS(X_matrix, Z_matrix, char_vec = char_vec, logs_alg = logs_alg)
...
function StabilizerCodeCSS(
    ...
    iszero(stabs) && throw(
        ArgumentError(
            "The processed Pauli strings returned a set of empty stabilizer generators.",
        ),
    )
    ...
    are_symplectic_orthogonal(stabs, stabs) ||
        throw(ArgumentError("The given stabilizers are not symplectic orthogonal."))
...
CSSCode(S::AbstractStabilizerCode; logs_alg::Symbol = :stnd_frm) =
    StabilizerCodeCSS(S; logs_alg = logs_alg)
...
function StabilizerCode(::IsCSS, S::AbstractStabilizerCode, logs_alg::Symbol)
    ...
    nrows(S.X_stabs) == nrows(S.Z_stabs) ||
        throw(ArgumentError("Requires an equal number of X and Z stabilizers"))

    n = div(S.n, 2)
    return StabilizerCode(hcat(S.X_stabs[:, 1:n], S.Z_stabs[:, 1:n]), logs_alg = logs_alg)
end
StabilizerCode(::IsNotCSS, S::AbstractStabilizerCode, logs_alg::Symbol) =
    throw(ArgumentError("Only valid for CSS codes of even length"))
...
"""
X_minimum_distance_lower_bound(S::T) where {T<:AbstractStabilizerCode} =
    X_minimum_distance_lower_bound(CSSTrait(T), S)
X_minimum_distance_lower_bound(::IsCSS, S::AbstractStabilizerCode) = S.l_bound_dx
X_minimum_distance_lower_bound(::IsNotCSS, S::AbstractStabilizerCode) =
...
"""
X_minimum_distance_upper_bound(S::T) where {T<:AbstractStabilizerCode} =
    X_minimum_distance_upper_bound(CSSTrait(T), S)
X_minimum_distance_upper_bound(::IsCSS, S::AbstractStabilizerCode) = S.u_bound_dx
X_minimum_distance_upper_bound(::IsNotCSS, S::AbstractStabilizerCode) =
    error("Only valid for CSS codes")
...
"""
Z_minimum_distance_lower_bound(S::T) where {T<:AbstractStabilizerCode} =
    Z_minimum_distance_lower_bound(CSSTrait(T), S)
Z_minimum_distance_lower_bound(::IsCSS, S::AbstractStabilizerCode) = S.l_bound_dz
Z_minimum_distance_lower_bound(::IsNotCSS, S::AbstractStabilizerCode) =
    error("Only valid for CSS codes")
...
"""
Z_minimum_distance_upper_bound(S::T) where {T<:AbstractStabilizerCode} =
    Z_minimum_distance_upper_bound(CSSTrait(T), S)
Z_minimum_distance_upper_bound(::IsCSS, S::AbstractStabilizerCode) = S.u_bound_dz
Z_minimum_distance_upper_bound(::IsNotCSS, S::AbstractStabilizerCode) =
    error("Only valid for CSS codes")
...
function set_minimum_distance!(S::AbstractStabilizerCode, d::Int)
    ...
    S.u_bound < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound)")
    d < S.l_bound &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound)")
    S.d = d
    S.l_bound = d
    S.u_bound = d
...
"""
set_X_minimum_distance!(S::T, d::Int) where {T<:AbstractStabilizerCode} =
    set_X_minimum_distance!(CSSTrait(T), S, d)
function set_X_minimum_distance!(::IsCSS, S::AbstractStabilizerCode, d::Int)
    0 < d <= S.n ||
        throw(DomainError("The minimum distance of a code must be ≥ 1; received: d = $d."))
    S.u_bound_dx < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dx)")
    d < S.l_bound_dx &&
...
"""
set_Z_minimum_distance!(S::T, d::Int) where {T<:AbstractStabilizerCode} =
    set_Z_minimum_distance!(CSSTrait(T), S, d)
function set_Z_minimum_distance!(::IsCSS, S::AbstractStabilizerCode, d::Int)
    0 < d <= S.n ||
        throw(DomainError("The minimum distance of a code must be ≥ 1; received: d = $d."))
    S.u_bound_dz < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dz)")
    d < S.l_bound_dz &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound_dz)")
    S.dz = d
    S.l_bound_dz = d
    S.u_bound_dz = d
...
function _logicals(
    ...
    n = div(ncols(L), 2)
    logs_mat = reduce(vcat, [reduce(vcat, logs[i]) for i = 1:length(logs)])
    are_symplectic_orthogonal(stabs, logs_mat) ||
...
function random_CSS_code(n::Int, k::Int)
    ...
    dt = transpose(d)
    Xrref = rref(dt)
    Xind = [findfirst(!iszero, Xrref[2][i, :])[2] for i = 1:Xrref[1]]
    Zrref = rref(d)
    Zind = [findfirst(!iszero, Zrref[2][i, :])[2] for i = 1:Zrref[1]]
...
"""
is_CSS_T_code(S::AbstractStabilizerCode; verbose::Bool = false) =
    is_CSS_T_code(CSSTrait(typeof(S)), S; verbose = verbose)
