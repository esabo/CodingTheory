...
"""
    SubsystemCode(G::CTMatrixTypes; char_vec::Union{Vector{zzModRingElem}, Missing} = missing, logs_alg::Symbol = :sys_eqs)
...
function SubsystemCode(
    ...
    ker_G_VS, ker_G_to_V = sub(V, [V(ker_G[i, :]) for i = 1:nrows(ker_G)])
    G_VS, _ = sub(V, [V(G[i, :]) for i = 1:nrows(G)])
    I, I_to_ker_G = intersect(ker_G_VS, G_VS)
    if !iszero(AbstractAlgebra.dim(I))
        I_basis = [ker_G_to_V(I_to_ker_G(g)) for g in gens(I)]
        F_basis = [
            [F(I_basis[j][i]) for i = 1:AbstractAlgebra.dim(parent(I_basis[1]))] for
            j = 1:length(I_basis)
        ]
        stabs = matrix(F, length(F_basis), 2 * n, reduce(vcat, F_basis))
    else
        error(
            "Error computing the stabilizer group of the subsystem code; ker G ∩ G has dimension zero.",
        )
    end
    ...
    if !iszero(BL)
        ...
        # verify
        bare_logs_mat =
            reduce(vcat, [reduce(vcat, bare_logs[i]) for i = 1:length(bare_logs)])
        are_symplectic_orthogonal(stabs, bare_logs_mat) ||
            error("Computed logicals do not commute with the codespace.")
        prod =
            hcat(bare_logs_mat[:, (n+1):end], -bare_logs_mat[:, 1:n]) *
            transpose(bare_logs_mat)
        sum(_Flint_matrix_to_Julia_int_matrix(prod), dims = 1) ==
        ones(Int, 1, size(prod, 1)) ||
    ...
    # verify
    gauge_ops_mat = reduce(vcat, [reduce(vcat, gauge_ops[i]) for i = 1:length(gauge_ops)])
    are_symplectic_orthogonal(stabs, gauge_ops_mat) ||
        error("Computed gauge operators do not commute with the codespace.")
    if !graph_state
        are_symplectic_orthogonal(bare_logs_mat, gauge_ops_mat) ||
            error("Computed gauge operators do not commute with the computed logicals.")
    end
    prod =
        hcat(gauge_ops_mat[:, (n+1):end], -gauge_ops_mat[:, 1:n]) * transpose(gauge_ops_mat)
    sum(_Flint_matrix_to_Julia_int_matrix(prod), dims = 1) == ones(Int, 1, size(prod, 1)) ||
        error("Computed gauge operators do not have the right commutation relations.")
    ...
    if args[1]
        if graph_state
            ...
            # dressed
            _, X_mat =
                rref(vcat(X_mat, reduce(vcat, [log[1][:, 1:n] for log in gauge_ops])))
            X_mat = _remove_empty(X_mat, :rows)
            u_bound_dx_dressed, _ = _min_wt_row(X_mat)

            _, Z_mat =
                rref(vcat(Z_mat, reduce(vcat, [log[2][:, (n+1):end] for log in gauge_ops])))
            Z_mat = _remove_empty(Z_mat, :rows)
            u_bound_dz_dressed, _ = _min_wt_row(Z_mat)
            return GraphStateSubsystemCSS(
                F,
                n,
                0,
                r,
                ...
                missing,
                1,
                min(u_bound_dx_bare, u_bound_dz_bare),
                1,
                min(u_bound_dx_dressed, u_bound_dz_dressed),
                1,
                u_bound_dx_bare,
                1,
                u_bound_dz_bare,
            ...
            u_bound_dx_bare, _ =
                _min_wt_row(X_mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :])
            _, Z_mat = rref(vcat(args[4], Z_logs))
            anti = _remove_empty(Z_mat, :rows) * transpose(X_logs)
            u_bound_dz_bare, _ =
                _min_wt_row(Z_mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :])

            # dressed
            _, X_mat =
                rref(vcat(X_mat, reduce(vcat, [log[1][:, 1:n] for log in gauge_ops])))
            anti = _remove_empty(X_mat, :rows) * transpose(Z_logs)
            u_bound_dx_dressed, _ =
                _min_wt_row(X_mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :])
            _, Z_mat =
                rref(vcat(Z_mat, reduce(vcat, [log[2][:, (n+1):end] for log in gauge_ops])))
            anti = _remove_empty(Z_mat, :rows) * transpose(X_logs)
            u_bound_dz_dressed, _ =
                _min_wt_row(Z_mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :])
        ...
        if graph_state
            ...
            anti =
                hcat(logs_mat[:, (n+1):end], -logs_mat[:, 1:n]) *
                transpose(_remove_empty(mat, :rows))
            u_bound_bare, _ = minimum(
                row_wts_symplectic(
                    mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :],
                ),
            )
...
"""
    SubsystemCode(G_Pauli::Vector{T}; char_vec::Union{Vector{zzModRingElem}, Missing} = missing, logs_alg::Symbol = :sys_eqs) where T <: Union{String, Vector{Char}}
...
function SubsystemCode(
    ...
    iszero(G) &&
        error("The processed Pauli strings returned a set of empty gauge group generators.")
...
function SubsystemCode(
    ...
    are_symplectic_orthogonal(S, S) ||
        error("The given stabilizers are not symplectic orthogonal.")
    ...
    cols = [sum(prod_Jul[:, i]) for i = 1:nc_pr]
    sum(cols) == nc_pr || println("Detected logicals not in anticommuting pairs.")
    log_pairs = _make_pairs(L)
    logs_mat = reduce(vcat, [reduce(vcat, log_pairs[i]) for i = 1:length(log_pairs)])
    ...
    are_symplectic_orthogonal(logs_mat, G) ||
        error("Gauges do not commute with the logicals.")
    ...
    g_ops_mat = reduce(vcat, [reduce(vcat, g_ops_pairs[i]) for i = 1:length(g_ops_pairs)])
    ...
    if args[1]
        ...
        u_bound_dx_dressed, _ =
            _min_wt_row(X_mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :])
        _, Z_mat =
            rref(vcat(Z_mat, reduce(vcat, [log[2][:, (n+1):end] for log in g_ops_pairs])))
        anti = _remove_empty(Z_mat, :rows) * transpose(X_logs)
        u_bound_dz_dressed, _ =
            _min_wt_row(Z_mat[findall(!iszero(anti[i:i, :]) for i in axes(anti, 1)), :])
        return SubsystemCodeCSS(
...
function SubsystemCode(
    ...
    iszero(S) &&
        error("The processed Pauli strings returned a set of empty stabilizer generators.")
    L = _Pauli_string_to_symplectic(_process_strings(L_Pauli))
    iszero(L) &&
        error("The processed Pauli strings returned a set of empty logical generators.")
...
"""
stabilizers(S::AbstractSubsystemCode; standform::Bool = false) =
    standform ? (return S.stabs_stand) : (return S.stabs)
...
"""
standard_form_A1(S::AbstractSubsystemCode) =
    S.stabs_stand[1:S.stand_r, (S.stand_r+1):(S.n-S.stand_k)]
...
"""
standard_form_A2(S::AbstractSubsystemCode) =
    S.stabs_stand[1:S.stand_r, (S.n-S.stand_k+1):S.n]
...
"""
standard_form_B(S::AbstractSubsystemCode) =
    S.stabs_stand[1:S.stand_r, (S.n+1):(S.n+S.stand_r)]
...
"""
standard_form_C1(S::AbstractSubsystemCode) =
    S.stabs_stand[1:S.stand_r, (S.n+S.stand_r+1):(2*S.n-S.stand_k)]
...
"""
standard_form_C2(S::AbstractSubsystemCode) =
    S.stabs_stand[1:S.stand_r, (2*S.n-S.stand_k+1):(2*S.n)]
...
"""
standard_form_D(S::AbstractSubsystemCode) =
    S.stabs_stand[(S.stand_r+1):(S.n-S.stand_k), (S.n+1):(S.n+S.stand_r)]
...
"""
standard_form_E(S::AbstractSubsystemCode) =
    S.stabs_stand[(S.stand_r+1):(S.n-S.stand_k), (2*S.n-S.stand_k+1):(2*S.n)]
...
logicals(::HasNoLogicals, S::AbstractSubsystemCode) =
...
logicals_matrix(::HasNoLogicals, S::AbstractSubsystemCode) =
    error("Type $(typeof(S)) has no logicals.")
...
"""
logicals_standard_form(S::T) where {T<:AbstractSubsystemCode} =
    logicals_standard_form(LogicalsTrait(T), S)
logicals_standard_form(::HasLogicals, S::AbstractSubsystemCode) =
    _logicals_standard_form(S.stabs_stand, S.n, S.stand_k, S.stand_r, S.P_stand)
logicals_standard_form(::HasNoLogicals, S::AbstractSubsystemCode) =
    error("Type $(typeof(S)) has no logicals.")
...
gauges_matrix(::HasNoGauges, S::AbstractSubsystemCode) =
...
gauge_group(::HasNoGauges, S::AbstractSubsystemCode) =
    error("Type $(typeof(S)) has no gauges.")
...
bare_minimum_distance_lower_bound(::HasNoGauges, S::AbstractSubsystemCode) = error(
    "Only valid for subsystem codes; use `minimum_distance_lower_bound` for stabilizer codes.",
)
...
bare_minimum_distance_upper_bound(::HasNoGauges, S::AbstractSubsystemCode) = error(
    "Only valid for subsystem codes; use `minimum_distance_lower_bound` for stabilizer codes.",
)
...
dressed_minimum_distance_lower_bound(::HasGauges, S::AbstractSubsystemCode) =
    S.l_bound_dressed
dressed_minimum_distance_lower_bound(::HasNoGauges, S::AbstractSubsystemCode) = error(
    "Only valid for subsystem codes; use `minimum_distance_lower_bound` for stabilizer codes.",
)
...
dressed_minimum_distance_upper_bound(::HasGauges, S::AbstractSubsystemCode) =
    S.u_bound_dressed
dressed_minimum_distance_upper_bound(::HasNoGauges, S::AbstractSubsystemCode) = error(
    "Only valid for subsystem codes; use `minimum_distance_lower_bound` for stabilizer codes.",
)
...
"""
bare_X_minimum_distance_lower_bound(S::T) where {T<:AbstractSubsystemCode} =
    bare_X_minimum_distance_lower_bound(GaugeTrait(T), CSSTrait(T), S)
...
bare_X_minimum_distance_lower_bound(::HasNoGauges, ::IsCSS, S::AbstractSubsystemCode) =
    error(
        "Only valid for subsystem codes; use `X_minimum_distance_lower_bound` for stabilizer codes.",
    )
...
"""
bare_X_minimum_distance_upper_bound(S::T) where {T<:AbstractSubsystemCode} =
    bare_X_minimum_distance_upper_bound(GaugeTrait(T), CSSTrait(T), S)
...
bare_X_minimum_distance_upper_bound(::HasNoGauges, ::IsCSS, S::AbstractSubsystemCode) =
    error(
        "Only valid for subsystem codes; use `X_minimum_distance_upper_bound` for stabilizer codes.",
    )
...
"""
dressed_X_minimum_distance_lower_bound(S::T) where {T<:AbstractSubsystemCode} =
    dressed_X_minimum_distance_lower_bound(GaugeTrait(T), CSSTrait(T), S)
...
"""
dressed_X_minimum_distance_upper_bound(S::T) where {T<:AbstractSubsystemCode} =
    dressed_X_minimum_distance_upper_bound(GaugeTrait(T), CSSTrait(T), S)
...
"""
bare_Z_minimum_distance_lower_bound(S::T) where {T<:AbstractSubsystemCode} =
    bare_Z_minimum_distance_lower_bound(GaugeTrait(T), CSSTrait(T), S)
...
bare_Z_minimum_distance_lower_bound(::HasNoGauges, ::IsCSS, S::AbstractSubsystemCode) =
    error(
        "Only valid for subsystem codes; use `Z_minimum_distance_lower_bound` for stabilizer codes.",
    )
...
"""
bare_Z_minimum_distance_upper_bound(S::T) where {T<:AbstractSubsystemCode} =
    bare_Z_minimum_distance_upper_bound(GaugeTrait(T), CSSTrait(T), S)
...
bare_Z_minimum_distance_upper_bound(::HasNoGauges, ::IsCSS, S::AbstractSubsystemCode) =
    error(
        "Only valid for subsystem codes; use `Z_minimum_distance_upper_bound` for stabilizer codes.",
    )
...
"""
dressed_Z_minimum_distance_lower_bound(S::T) where {T<:AbstractSubsystemCode} =
    dressed_Z_minimum_distance_lower_bound(GaugeTrait(T), CSSTrait(T), S)
...
"""
dressed_Z_minimum_distance_upper_bound(S::T) where {T<:AbstractSubsystemCode} =
    dressed_Z_minimum_distance_upper_bound(GaugeTrait(T), CSSTrait(T), S)
...
function set_signs!(S::AbstractSubsystemCode, char_vec::Vector{zzModRingElem})
    ...
    length(char_vec) == 2 * S.n ||
        throw(ArgumentError("Characteristic vector is of improper length for the code."))
    for s in char_vec
        modulus(s) == modulus(R) ||
            throw(ArgumentError("Phases are not in the correct ring."))
    end
...
set_signs(S::AbstractSubsystemCode, char_vec::Vector{zzModRingElem}) =
    (S_new = deepcopy(S); return setsign!s(S_new, char_vec))
...
function set_stabilizers!(S::AbstractSubsystemCode, stabs::CTMatrixTypes)
    ...
    order(S.F) == order(base_ring(stabs)) ||
        throw(ArgumentError("The stabilizers must be over the same field as the code."))
...
set_stabilizers(S::AbstractSubsystemCode, stabs::CTMatrixTypes) =
    (S_new = deepcopy(S); set_stabilizers!(S_new, stabs); return S_new)
...
set_X_stabilizers!(
    ...
    trimmed::Bool = true,
) where {T<:AbstractSubsystemCode} = set_X_stabilizers!(CSSTrait(T), S, X_stabs, trimmed)
function set_X_stabilizers!(
    ::IsCSS,
    S::AbstractSubsystemCode,
    X_stabs::CTMatrixTypes,
    trimmed::Bool,
    ...
    order(S.F) == order(base_ring(X_stabs)) ||
        throw(ArgumentError("The stabilizers must be over the same field as the code."))
    if trimmed
        ...
        ncols(X_stabs) == 2 * S.n ||
            throw(ArgumentError("Trimmed not set and input of wrong size"))
    ...
    S.stabs = vcat(
        hcat(X_trimmed, zero_matrix(S.F, nrows(X_trimmed), S.n)),
...
set_X_stabilizers(
    ...
    S_new = deepcopy(S);
    set_X_stabilizers!(IsCSS(), S_new, X_stabs, trimmed);
    return S_new
)
set_X_stabilizers(
    ::IsNotCSS,
...
set_Z_stabilizers!(
    ...
    trimmed::Bool = true,
) where {T<:AbstractSubsystemCode} = set_Z_stabilizers!(CSSTrait(T), S, Z_stabs, trimmed)
function set_Z_stabilizers!(
    ::IsCSS,
    S::AbstractSubsystemCode,
    Z_stabs::CTMatrixTypes,
    trimmed::Bool,
    ...
    order(S.F) == order(base_ring(Z_stabs)) ||
        throw(ArgumentError("The stabilizers must be over the same field as the code."))
    if trimmed
    ...
    S.stabs = vcat(
        hcat(S.X_stabs, zero_matrix(S.F, nrows(S.X_stabs), S.n)),
...
set_Z_stabilizers(
    ...
    S_new = deepcopy(S);
    set_Z_stabilizers!(IsCSS(), S_new, Z_stabs, trimmed);
    return S_new
)
set_Z_stabilizers(
    ::IsNotCSS,
...
function set_logicals!(
    ::HasLogicals,
    S::AbstractSubsystemCode,
    L::W,
) where {W<:CTMatrixTypes}
    size(L) == (2 * S.k, 2 * S.n) ||
        throw(ArgumentError("Provided matrix is of incorrect size for the logical space."))
    iseven(ncols(L)) || throw(
        ArgumentError(
            "Expected a symplectic input but the input matrix has an odd number of columns.",
        ),
    )
    S.F == base_ring(L) ||
        throw(ArgumentError("The logicals must be over the same field as the code."))

    _has_equivalent_row_spaces(vcat(S.logs_mat, S.stabs), vcat(L, S.stabs)) ||
    ...
    iszero(prod) &&
        throw(ArgumentError("Provided logicals should not be symplectic self-orthogonal."))
    nc_pr = ncols(prod)
    # need an integer sum and this is cheaper than Nemo.ZZ
    prod_Jul = _Flint_matrix_to_Julia_int_matrix(prod)
    cols = [sum(prod_Jul[:, i]) for i = 1:nc_pr]
    sum(cols) == nc_pr || throw(
        ArgumentError("Incorrect commutation relationships between provided logicals."),
    )
    ...
    S.logicals = logs
    S.logs_mat = reduce(vcat, [reduce(vcat, logs[i]) for i = 1:length(logs)])
end
...
function set_bare_minimum_distance!(::HasGauges, S::AbstractSubsystemCode, d::Int)
    ...
    S.u_bound_bare < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_bare)")
    d < S.l_bound_bare &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound_bare)")
    S.d_bare = d
    S.l_bound_bare = d
    S.u_bound_bare = d
    ...
    if ismissing(S.d_dressed)
        ...
        S.d_dressed ≤ S.d_bare ||
            (@warn "The bare distance is a bound on the dressed distance, but this is false for the new set parameters.")
    end
...
set_bare_minimum_distance!(::HasNoGauges, S::AbstractSubsystemCode, d::Int) = error(
    "Only valid for subsytem codes; use `set_minimum_distance!` for stabilizer codes.",
)
...
"""
set_bare_X_minimum_distance!(S::T, d::Int) where {T<:AbstractSubsystemCode} =
    set_bare_X_minimum_distance!(GaugeTrait(T), CSSTrait(T), S, d)
function set_bare_X_minimum_distance!(
    ::HasGauges,
    ::IsCSS,
    S::AbstractSubsystemCode,
    d::Int,
    ...
    0 < d <= S.n ||
        throw(DomainError("The minimum distance of a code must be ≥ 1; received: d = $d."))
    S.u_bound_dx_bare < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dx_bare)")
    d < S.l_bound_dx_bare &&
...
"""
set_bare_Z_minimum_distance!(S::T, d::Int) where {T<:AbstractSubsystemCode} =
    set_bare_Z_minimum_distance!(GaugeTrait(T), CSSTrait(T), S, d)
function set_bare_Z_minimum_distance!(
    ::HasGauges,
    ::IsCSS,
    S::AbstractSubsystemCode,
    d::Int,
    ...
    0 < d <= S.n ||
        throw(DomainError("The minimum distance of a code must be ≥ 1; received: d = $d."))
    S.u_bound_dz_bare < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dz_bare)")
    d < S.l_bound_dz_bare &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound_dz_bare)")
    S.dz_bare = d
    S.l_bound_dz_bare = d
    S.u_bound_dz_bare = d
...
set_bare_Z_minimum_distance!(::HasNoGauges, ::IsCSS, S::AbstractSubsystemCode, d::Int) =
    error(
        "Only valid for subsytem codes; use `set_Z_minimum_distance!` for stabilizer codes.",
    )
...
function set_dressed_minimum_distance!(::HasGauges, S::AbstractSubsystemCode, d::Int)
    0 < d <= S.n ||
        throw(DomainError("The minimum distance of a code must be ≥ 1; received: d = $d."))
    S.u_bound_dressed < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dressed)")
    d < S.l_bound_dressed &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound_dressed)")
    S.d_dressed = d
    S.l_bound_dressed = d
    S.u_bound_dressed = d
...
set_dressed_minimum_distance!(::HasNoGauges, S::AbstractSubsystemCode, d::Int) = error(
    "Only valid for subsytem codes; use `set_minimum_distance!` for stabilizer codes.",
)
...
"""
set_dressed_X_minimum_distance!(S::T, d::Int) where {T<:AbstractSubsystemCode} =
    set_dressed_X_minimum_distance!(GaugeTrait(T), CSSTrait(T), S, d)
function set_dressed_X_minimum_distance!(
    ::HasGauges,
    ::IsCSS,
    S::AbstractSubsystemCode,
    d::Int,
    ...
    S.u_bound_dx_dressed < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dx_dressed)")
    d < S.l_bound_dx_dressed &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound_dx_dressed)")
    S.dx_dressed = d
...
"""
set_dressed_Z_minimum_distance!(S::T, d::Int) where {T<:AbstractSubsystemCode} =
    set_dressed_Z_minimum_distance!(GaugeTrait(T), CSSTrait(T), S, d)
function set_dressed_Z_minimum_distance!(
    ::HasGauges,
    ::IsCSS,
    S::AbstractSubsystemCode,
    d::Int,
    ...
    S.u_bound_dz_dressed < d &&
        (@warn "The distance set is greater than the current upper bound of $(S.u_bound_dz_dressed)")
    d < S.l_bound_dz_dressed &&
        (@warn "The distance set is less than the current lower bound of $(S.l_bound_dz_dressed)")
    S.dz_dressed = d
...
function _process_char_vec(char_vec::Union{Vector{zzModRingElem},Missing}, p::Int, n::Int)
    if !ismissing(char_vec)
        n == length(char_vec) ||
            throw(ArgumentError("The characteristic value is of incorrect length."))
        if p == 2
...
function _determine_signs_CSS(
    ...
    if iszero(char_vec)
        R = parent(char_vec[1])
        signs = [R(0) for _ = 1:nrows(S)]
        X_signs = [R(0) for _ = 1:X_size]
        Z_signs = [R(0) for _ = 1:Z_size]
    else
        signs = _get_signs(S, char_vec)
...
function _get_signs(A::CTMatrixTypes, char_vec::Vector{zzModRingElem})
    ...
    nc = ncols(A)
    length(char_vec) == nc || throw(
        ArgumentError(
            "Input to _get_signs is expected to be in symplectic form and of the same length as the characteristic vector.",
        ),
    )

    iszero(char_vec) && return [R(0) for _ = 1:div(nc, 2)]
    signs = Vector{Int64}()
    for r = 1:nrows(A)
        parity = R(0)
        for c = 1:nc
...
function _split_vectors_CSS(S::T, signs::Vector{zzModRingElem}) where {T<:CTMatrixTypes}
    X_stabs = Vector{T}()
...
function _is_CSS_symplectic(
    stabs::T,
    signs::Vector{zzModRingElem},
    trim::Bool = true,
) where {T<:CTMatrixTypes}
    X_stabs, X_signs, Z_stabs, Z_signs, mixed_stabs, mixed_signs =
        _split_vectors_CSS(stabs, signs)
...
function _make_pairs(L::T) where {T<:CTMatrixTypes}
    ...
    if Int(order(F)) != 2
        ...
        while nrows(L) >= 2
            ...
            iszero(first) && error(
                "Cannot make symplectic basis. Often this is due to the fact that the stabilizers are not maximal and therefore the centralizer still containing part of the isotropic subspace.",
            )
            for c = 2:num_prod
...
_test_logicals_relationships(::HasNoLogicals, S) =
    error("Type $(typeof(S)) has no logicals.")
...
"""
is_logical(S::T, v::CTMatrixTypes) where {T<:AbstractSubsystemCode} =
    is_logical(LogicalTrait(T), S, v)
...
"""
is_gauge(S::T, v::CTMatrixTypes) where {T<:AbstractSubsystemCode} =
    is_gauge(GaugeTrait(T), S, v)
...
is_gauge(::HasNoGauges, S::AbstractSubsystemCode, v::CTMatrixTypes) =
    error("Type $(typeof(S)) has no gauges.")
...
function syndrome(S::AbstractSubsystemCode, v::CTMatrixTypes)
    (size(v) != (2 * S.n, 1) && size(v) != (1, 2 * S.n)) && throw(
        ArgumentError(
            "Vector to be tested is of incorrect dimension; expected length $(2 * n), received: $(size(v)).",
        ),
    )
...
CSS code.
"""
X_syndrome(S::T, v::CTMatrixTypes) where {T<:AbstractSubsystemCode} =
    X_syndrome(CSSTrait(T), S, v)
function X_syndrome(::IsCSS, S::AbstractSubsystemCode, v::CTMatrixTypes)
    length(v) == 2 * S.n && (v = v[(S.n+1):end];)
    (size(v) != (S.n, 1) && size(v) != (1, S.n)) && error(
        "Vector to be tested is of incorrect dimension; expected length $n, received: $(size(v)).",
    )
...
CSS code.
"""
Z_syndrome(S::T, v::CTMatrixTypes) where {T<:AbstractSubsystemCode} =
    Z_syndrome(CSSTrait(T), S, v)
function Z_syndrome(::IsCSS, S::AbstractSubsystemCode, v::CTMatrixTypes)
    length(v) == 2 * S.n && (v = v[1:S.n];)
    (size(v) != (n, 1) && size(v) != (1, n)) && error(
        "Vector to be tested is of incorrect dimension; expected length $n, received: $(size(v)).",
    )
    base_ring(v) == S.F || error("Vector must have the same base ring as the stabilizers.")
...
function promote_logicals_to_gauge!(
    ::HasLogicals,
    S::AbstractSubsystemCode,
    pairs::Vector{Int},
)
    ...
    S.g_ops_mat = reduce(vcat, [reduce(vcat, S.gauge_ops[i]) for i = 1:length(S.gauge_ops)])
    S.logicals = S.logicals[setdiff!(append!(collect(1:length(S.logicals)), pairs))]
    S.logs_mat = reduce(vcat, [reduce(vcat, S.logicals[i]) for i = 1:length(S.logicals)])
...
promote_logicals_to_gauge(::HasLogicals, S::AbstractSubsystemCode, pairs::Vector{Int}) = (
    S_new = deepcopy(S);
    promote_logicals_to_gauge!(HasLogicals(), S_new, pairs);
    return S_new
)
promote_logicals_to_gauge(::HasNoLogicals, S::AbstractSubsystemCode, pairs::Vector{Int}) =
    error("Type $(typeof(S)) has no logicals.")
...
function promote_gauges_to_logical!(
    ::HasLogicals,
    S::AbstractSubsystemCode,
    pairs::Vector{Int},
)
...
promote_gauges_to_logical(::HasLogicals, S::AbstractSubsystemCode, pairs::Vector{Int}) = (
    S_new = deepcopy(S);
    promote_gauges_to_logical!(HasLogicals(), S_new, pairs);
    return S_new
)
promote_gauges_to_logical(::HasNoLogicals, S::AbstractSubsystemCode, pairs::Vector{Int}) =
    error("Type $(typeof(S)) has no logicals.")
...
function swap_X_Z_gauge_operators!(
    ::HasGauges,
    S::AbstractSubsystemCode,
    pairs::Vector{Int},
)
...
function are_equivalent(S1::T, S2::T) where {T<:AbstractSubsystemCode}
    ...
    if LogicalTrait(T) == HasLogicals()
        # test logicals
        _has_equivalent_row_spaces(
            vcat(S1.logs_mat, S1.stabs),
            vcat(S2.logs_mat, S2.stabs),
        ) || return false
    end

    if GaugeTrait(T) == HasGauges()
        # test gauge operators
        return _has_equivalent_row_spaces(
            vcat(S1.g_ops_mat, S1.stabs),
            vcat(S2.g_ops_mat, S2.stabs),
        )
    else
        return true
    end
...
function show(io::IO, S::AbstractSubsystemCode)
    ...
    if get(io, :compact, true) && S.n <= 30
        if isa(S, SubsystemCodeCSS) ||
           isa(S, StabilizerCodeCSS) ||
           isa(S, GraphStateStabilizerCSS) ||
...
function _all_stabilizers(S::AbstractStabilizerCode, only_print::Bool = false)
    ...
    stabs = S.stabs
    for iter in
        Base.Iterators.product([0:(Int64(characteristic(S.F))-1) for _ = 1:nrows(stabs)]...)
...
"""
function permute_code!(
    S::AbstractSubsystemCode,
    σ::Union{PermGroupElem,Perm{Int},Vector{Int}},
)
...
function augment(
    ...
    logs_alg ∈ (:sys_eqs, :VS) || throw(ArgumentError("Unrecognized logicals algorithm"))
    typeof(S.stabs) == typeof(row) ||
        throw(ArgumentError("Vector of different (Julia) type than stabilizers"))
    iszero(row) && return S
    nrows(row) == 1 ||
        throw(ArgumentError("Only one stabilizer may be passed in at a time."))
    ...
    if iszero(prod)
        ...
        for i = 1:nrows(S.stabs)
            iszero(prod[i]) && append!(stabs_to_keep, i, i + 1)
        end

        if isempty(stabs_to_keep)
            verbose && println(
                "The vector anticommutes with all stabilizers. The new stabilizer group is just the vector.",
            )
            stabs = row
        else
    ...
    if GaugeTrait(typeof(S)) == HasGauges()
        ...
        if isempty(g_ops_to_keep)
            ...
            isempty(update) ? (gauge_ops = S.gauge_ops;) :
            (gauge_ops = S.gauge_ops[g_ops_to_keep, :];)
        end
    else
        gauge_ops = zero_matrix(S.F, 1, 2 * S.n)
    end
...
function expurgate(
    ...
    num_stabs = nrows(S.stabs)
    rows ⊆ 1:num_stabs ||
        throw(ArgumentError("Argument `rows` not a subset of the number of stabilizers."))
    ...
    if iszero(new_logs)
        ...
        rank(new_stabs) == nrows(new_stabs) ? (S.overcomplete = false;) :
        (S.overcomplete = true;)
        return S_new
    else
        ...
        if GaugeTrait(typeof(S)) == HasGauges()
            return SubsystemCode(
                new_stabs,
                vcat(S.logs_mat, reduce(vcat, reduce(vcat, new_log_pairs))),
                S.g_ops_mat,
                char_vec = S.char_vec,
            )
        else
...
function _logicals_standard_form(
    stabs::CTMatrixTypes,
    n::Int,
    k::Int,
    r::Int,
    P::Union{Missing,CTMatrixTypes},
)
