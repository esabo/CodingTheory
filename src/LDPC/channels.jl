...
"""
erasure_probability(Ch::BinaryErasureChannel) = _param(Ch)
...
"""
crossover_probability(Ch::BinarySymmetricChannel) = _param(Ch)
...
"""
standard_deviation(Ch::BAWGNChannel) = _param(Ch)
...
"""
variance(Ch::BAWGNChannel) = _param(Ch)^2
...
"""
capacity(Ch::BinaryErasureChannel) = 1.0 - _param(Ch)
capacity(Ch::BinarySymmetricChannel) = 1.0 - _binary_entropy(_param(Ch))
...
function capacity(Ch::RayleighFadingChannel)
    ...
    f(a) = 2.0 * a * exp(-a^2) * capacity(BAWGNChannel(_param(Ch) / a))
...
# Fallback for channels where capacity is not yet analytically or numerically defined
capacity(::AbstractChannel) = missing
...
Return a tuple `(y, a)` where `y` is the received signal and `a` is the channel state information (fading coefficients).
"""
...
Return the exact log-likelihood ratios (LLRs) for a received Rayleigh fading
signal, where `received` is the tuple `(y, a)` returned by `transmit`.
"""
...
function show(io::IO, Ch::AbstractChannel)
    if isa(Ch, BinaryErasureChannel)
        print(io, "Binary erasure channel with erasure probability $(_param(Ch))")
    elseif isa(Ch, BinarySymmetricChannel)
            print(io, "Binary symmetric channel with crossover probability $(_param(Ch))")
    elseif isa(Ch, BAWGNChannel)
        print(io, "Binary (input) additive white Gaussian noise channel with standard deviation $(_param(Ch))")
    else
        print(io, "Classical noise channel with parameter $(_param(Ch))")
    end

    # # Evaluate capacity on the fly
    # cap = capacity(Ch)
    
    # if !ismissing(cap)
    #     println(io, " and capacity $(cap).")
    # else
    #     println(io, ".")
    # end
end
