                        ...
                        for cn in var_adj_list[v]
                            if cn != prev
                                if iter != 1 && vn ∈ check_adj_list[cn]
                                    ...
                                    push!(
                                        to_check_next,
                                        (cn, [v2 for v2 in check_adj_list[cn] if v2 != v]),
                                    )
                                end
                            end
                        end
...
function remove_cycles(H::Union{Matrix{<: Integer},CTMatrixTypes}, n_max::Int)
    ...
    size(H, 1) ≥ 1 && size(H, 2) ≥ 1 ||
        throw(ArgumentError("Matrix cannot have a zero dimension"))
                ...
                if i ≠ j
                    if matrices[n2+1][i, j] ≥ 2 && iszero(matrices[n2m2+1][i, j])
                        ...
                        if !isempty(temp)
                            ...
                            C_e = Int[]
                            for x = 1:nc
                                if iszero(matrices[nm1+1][j, x]) &&
                                   iszero(matrices[nm1+1][k, x])
                                    append!(C_e, x)
                                end
                            end
                            ...
                            if !isempty(E_e)
                                ...
                                for indx = 2:(n_max-1)
                                    for c = 1:nc
                                        matrices[indx+1][j, c] = dot(
                                            view(matrices[indx-1+1], j, :),
                                            view(matrices[1+1], :, c),
                                        )
                                        matrices[indx+1][k, c] = dot(
                                            view(matrices[indx-1+1], k, :),
                                            view(matrices[1+1], :, c),
                                        )
                                        matrices[indx+1][l, c] = dot(
                                            view(matrices[indx-1+1], l, :),
                                            view(matrices[1+1], :, c),
                                        )
                                        matrices[indx+1][m, c] = dot(
                                            view(matrices[indx-1+1], m, :),
                                            view(matrices[1+1], :, c),
                                        )
                                    end

                                    for r = 1:nc
                                        matrices[indx+1][r, j] = dot(
                                            view(matrices[indx-1+1], r, :),
                                            view(matrices[1+1], :, j),
                                        )
                                        matrices[indx+1][r, k] = dot(
                                            view(matrices[indx-1+1], r, :),
                                            view(matrices[1+1], :, k),
                                        )
                                        matrices[indx+1][r, l] = dot(
                                            view(matrices[indx-1+1], r, :),
                                            view(matrices[1+1], :, l),
                                        )
                                        matrices[indx+1][r, m] = dot(
                                            view(matrices[indx-1+1], r, :),
                                            view(matrices[1+1], :, m),
                                        )
                                    end
...
remove_cycles(L::LDPCCode, n_max::Int) =
    LDPCCode(remove_cycles(parity_check_matrix(L), n_max))
...
function _modified_hawick_james(g::DiGraph{Int}, num_var_nodes::Int, max_len::Int = 16)

    # since this is bipartite, the input parameter tells you which are var nodes and which are
    # check nodes
    nvg = Graphs.nv(g)
    local_cycles = [Vector{Vector{Int}}() for _ = 1:num_var_nodes]
    Threads.@threads for i = 1:num_var_nodes
...
function enumerate_simple_cycles(L::AbstractLDPCCode; len::Int = 16)
    ispositive(len) ||
        throw(DomainError("Cycle length parameter must be a positive integer"))
...
"""
simple_cycle_length_distribution(L::AbstractLDPCCode; len::Int = 16) =
    StatsBase.countmap(length.(enumerate_simple_cycles(L, len = len)))
...
"""
average_simple_cycle_length(L::AbstractLDPCCode; len::Int = 16) =
    mean(length.(enumerate_simple_cycles(L, len = len)))
...
"""
median_simple_cycle_length(L::AbstractLDPCCode; len::Int = 16) =
    median(length.(enumerate_simple_cycles(L, len = len)))
...
"""
mode_simple_cycle_length(L::AbstractLDPCCode; len::Int = 16) =
    StatsBase.mode(length.(enumerate_simple_cycles(L, len = len)))
...
"""
count_simple_cycles(L::AbstractLDPCCode; len::Int = 16) =
    length(enumerate_simple_cycles(L, len = len))
...
simple_cycle_distribution_by_variable_node(L::AbstractLDPCCode; len::Int = 16) =
    StatsBase.countmap(
        filter(x -> x ≤ L.n, reduce(vcat, enumerate_simple_cycles(L, len = len))),
    )
...
"""
enumerate_short_cycles(L::AbstractLDPCCode; len::Int = 16) =
    filter(x -> length(x) <= 2 * girth(L) - 2, _enumerate_cycles(L, len))
...
"""
short_cycle_length_distribution(L::AbstractLDPCCode; len::Int = 16) =
    StatsBase.countmap(length.(enumerate_short_cycles(L, len = len)))
...
"""
average_short_cycle_length(L::AbstractLDPCCode; len::Int = 16) =
    mean(length.(enumerate_short_cycles(L, len = len)))
...
"""
median_short_cycle_length(L::AbstractLDPCCode; len::Int = 16) =
    median(length.(enumerate_short_cycles(L, len = len)))
...
"""
mode_short_cycle_length(L::AbstractLDPCCode; len::Int = 16) =
    StatsBase.mode(length.(enumerate_short_cycles(L, len = len)))
...
"""
count_short_cycles(L::AbstractLDPCCode; len::Int = 16) =
    length(enumerate_short_cycles(L, len = len))
...
short_cycle_distribution_by_variable_node(L::AbstractLDPCCode; len::Int = 16) =
    StatsBase.countmap(
        filter(x -> x ≤ L.n, reduce(vcat, enumerate_short_cycles(L, len = len))),
    )
...
function shortest_cycle_ACE(C::AbstractLDPCCode, vs::Vector{Int})
    ...
    all(x -> 1 <= x <= C.n, vs) ||
        throw(DomainError("Variable node indices must be between 1 and length(C)"))

    # might not be efficient to have the or here
    vs_to_do =
        [x for x in vs if isempty(C.ACEs_per_var_node[x]) || isempty(C.shortest_cycles[x])]
    processed = false
    if !isempty(vs_to_do)
        processed = true
        check_adj_list, var_adj_list = _node_adjacencies(C.H)

        Threads.@threads for i = 1:length(vs_to_do)
            # moving this inside allocates more but allows for multi-threading
            check_nodes = [_ACECheckNode(j, -1, -1, -1) for j = 1:length(check_adj_list)]
            var_nodes =
                [_ACEVarNode(j, -1, -1, -1, length(var_adj_list[j]) - 2) for j = 1:C.n]
    ...
    cycles_vs = [Vector{Tuple{Int,Int}}() for _ = 1:length(vs)]
    for i = 1:length(vs)
    ...
    if processed
        if all(!isempty, C.cycle_lens)
            girth = minimum([minimum(C.cycle_lens[i]) for i = 1:C.n])
...
function ACE_distribution(C::AbstractLDPCCode, vs::Vector{Int})
    ...
    all(x -> 1 <= x <= C.n, vs) ||
        throw(DomainError("Variable node index must be between 1 and length(C)"))
    ...
    if !isempty(vs_to_do)
        ...
        Threads.@threads for i = 1:length(vs_to_do)
            # moving this inside allocates more but allows for multi-threading
            check_nodes = [_ACECheckNode(i, -1, -1, -1) for i = 1:length(check_adj_list)]
            var_nodes =
                [_ACEVarNode(i, -1, -1, -1, length(var_adj_list[i]) - 2) for i = 1:C.n]
...
function ACE_spectrum(C::AbstractLDPCCode)
    ...
    counts = [Dict{Int,Int}() for _ = 1:length(girth:2:(2*girth-2))]
    for (k, l) in enumerate(girth:2:(2*girth-2))
        for i = 1:length(shortest_lens)
