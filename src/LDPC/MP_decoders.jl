...
function _Gallager_A_check_node_message(
    ...
    var_to_check_messages::Array{Int,3},
    attenuation::Float64,
)

    @inbounds reduce(
        ⊻,
        var_to_check_messages[v2, c, iter] for v2 in check_adj_list[c] if v2 != v
    )
end
...
function Gallager_A(
    ...
    schedule ∈ (:flooding, :parallel, :serial) ||
    ...
    var_to_check_messages,
    current_bits,
    syn = _message_passing_init_Int(H, v, max_iter, :A, 2, schedule, Int[])

    return _message_passing_Int(
        H_Int,
        missing,
        chn_inits_2,
        _Gallager_A_check_node_message,
...
function Gallager_B(
    ...
    var_to_check_messages,
    current_bits,
    syn = _message_passing_init_Int(H, v, max_iter, :B, threshold, schedule, Int[])
...
function sum_product(
    ...
    var_to_check_messages,
    current_bits,
    syn = _message_passing_init_fast(H, v, chn, :SP, chn_inits, :serial, erasures)
    if schedule == :layered
        # initialization - do these outside to reduce allocations when looped
...
function sum_product_box_plus(
    ...
    current_bits,
    totals,
    syn = _message_passing_init(H, v, chn, :SP, chn_inits, schedule, erasures)
    return _message_passing_layered(
        H_Int,
        missing,
        chn_inits_2,
...
function sum_product_syndrome(
    ...
    nr, nc = size(H)
    (nr ≥ 0 && nc ≥ 0) || throw(ArgumentError("`H` cannot have a zero dimension"))
    (size(syndrome) ≠ (nr, 1) && size(syndrome) ≠ (1, nr)) &&
        throw(ArgumentError("Syndrome has incorrect dimension"))
    # do we want to flip it if necessary?
    2 ≤ max_iter || throw(DomainError("Maximum number of iterations must be at least two"))
    schedule ∈ (:flooding, :parallel, :serial, :layered, :semiserial) ||
        throw(ArgumentError("Unknown schedule algorithm"))
    ...
    current_bits,
    totals,
    syn = _message_passing_init(H, syndrome, chn, :SP, chn_inits, schedule, erasures)
...
function sum_product_decimation(
    H::T,
    v::T,
    chn::AbstractClassicalNoiseChannel,
    algorithm::Symbol;
    decimated_bits_values::Vector{Tuple{Int,S}} = Tuple{Int,S}[],
    max_iter::Int = 100,
    chn_inits::Union{Missing,Vector{Float64}} = missing,
    schedule::Symbol = :parallel,
    rand_sched::Bool = false,
    ...
    erasures::Vector{Int} = Int[],
) where {T<:CTMatrixTypes,S<:CTFieldElem}
    ...
    schedule == :flooding && (schedule = :parallel;)
    schedule == :semiserial && (schedule = :layered;)
    algorithm ∈ (:auto, :manual, :guided) ||
        throw(ArgumentError("Unknown decimation algorithm"))
    ...
    (algorithm == :auto || algorithm == :guided) &&
        (decimated_bits_values = Tuple{Int,S}[];)
    if algorithm == :guided
        guided_rounds > 0 ||
            throw(DomainError("The number of rounds before decimation must be positive"))
    end
    ...
    current_bits,
    totals,
    syn = _message_passing_init_decimation(
        H,
        v,
        chn,
        decimated_bits_values,
        :SP,
        2,
        chn_inits,
        schedule,
    ...
    # TODO layers
    return _message_passing_decimation(
        H_Int,
        w,
        chn_inits_2,
        _SP_check_node_message,
...
function min_sum_syndrome(
    ...
    current_bits,
    totals,
    syn = _message_passing_init(H, syndrome, chn, :MS, chn_inits, schedule, erasures)
    syn_Int = _Flint_matrix_to_Julia_int_vector(syndrome)
    return _message_passing_layered(
        H_Int,
        syn_Int,
...
box_plus_min_c(a::Float64, b::Float64) =
    sign(a) * sign(b) * min(abs(a), abs(b)) + _min_sum_corr_s(a, b)
box_plus_min_c(a...) = reduce(box_plus_min_c, a...)
function _MS_correction_check_node_message(
    c::Int,
    v::Int,
    iter::Int,
    check_adj_list::Vector{Vector{Int}},
...
"""
function min_sum_with_correction(
    H::T,
    v::T,
    chn::AbstractClassicalNoiseChannel;
    max_iter::Int = 100,
...
- A low-complexity approximation to the correction term is used.
"""
function min_sum_with_correction_syndrome(
    H::T,
    syndrome::T,
    chn::AbstractClassicalNoiseChannel;
    max_iter::Int = 100,
...
function _channel_init_BSC!(
    var_to_check_messages::Matrix{Float64},
    v::CTMatrixTypes,
    p::Float64,
)
    temp = log((1 - p) / p)
    @inbounds for i = 1:length(v)
        iszero(v[i]) ? (var_to_check_messages[i, 1] = temp;) :
        (var_to_check_messages[i, 1] = -temp;)
    end
    return nothing
end
...
function _channel_init_BAWGNC_SP!(
    var_to_check_messages::Matrix{Float64},
...
_channel_init_BAWGNC_MS!(
    var_to_check_messages::Matrix{Float64},
    v::Vector{<: AbstractFloat},
) = var_to_check_messages[:, 1] .= v

function _message_passing_init_fast(
    H::Union{Matrix{S},T},
    v::Union{Vector{S},Vector{AbstractFloat},T},
    chn::AbstractClassicalNoiseChannel,
    kind::Symbol,
    chn_inits::Union{Missing,Vector{Float64}},
    schedule::Symbol,
    erasures::Vector{Int},
) where {S<:Integer,T<:CTMatrixTypes}
    ...
    check_to_var_messages::Vector{Vector{Float64}} =
        [zeros(Float64, length(check_adj_list[c])) for c in eachindex(check_adj_list)]
    if schedule == :serial
...
function _message_passing_init(
    ...
    check_adj_list = [Int[] for _ = 1:num_check]
    var_adj_list = [Int[] for _ = 1:num_var]
    for r = 1:num_check
        for c = 1:num_var
            if !iszero(H_Int[r, c])
                push!(check_adj_list[r], c)
...
function _message_passing_init_Int(
    H::Union{Matrix{S},T},
    v::Union{Vector{S},Vector{AbstractFloat},T},
    max_iter::Int,
    kind::Symbol,
...
function _message_passing_init_decimation(
    H::T,
    v::T,
    chn::AbstractClassicalNoiseChannel,
    decimated_bits_values::Vector{Tuple{Int,S}},
    kind::Symbol,
    Bt::Int,
    chn_inits::Union{Missing,Vector{Float64}},
    schedule::Symbol,
    erasures::Vector{Int},
) where {S<:CTFieldElem,T<:CTMatrixTypes}
...
function _message_passing(
    ...
    if !ismissing(syndrome)
        all(syn[i] .% 2 == syndrome[i] for i = 1:num_check) &&
            return true, current_bits, 1, totals
    else
        all(iszero(syn[i] .% 2) for i = 1:num_check) && return true, current_bits, 1, totals
    end
    ...
    @inbounds while iter ≤ max_iter
        @simd for v = 1:num_var
            for c in var_adj_list[v]
                ...
                var_to_check_messages[v, c, curr_iter] =
                    totals[v] - check_to_var_messages[c, v, prev_iter]
            end
        end

        @simd for c = 1:num_check
            for v in check_adj_list[c]
                if !ismissing(syndrome)
                    check_to_var_messages[c, v, curr_iter] =
                        (-1)^syndrome[c] * c_to_v_mess(
                            c,
                            v,
                            curr_iter,
                            check_adj_list,
                    ...
                    check_to_var_messages[c, v, curr_iter] = c_to_v_mess(
                        c,
                        v,
                        curr_iter,
                        check_adj_list,
        ...
        if !ismissing(syndrome)
            all(syn[i] .% 2 == syndrome[i] for i = 1:num_check) &&
                return true, current_bits, iter, totals
        else
            all(iszero(syn[i] .% 2) for i = 1:num_check) &&
                return true, current_bits, iter, totals
        end
...
function _message_passing_layered(
    H::Matrix{T},
    syndrome::Union{Missing,Vector{T}},
    chn_inits::Vector{Float64},
    c_to_v_mess::Function,
...
function _message_passing_fast(
    H_Int::Matrix{UInt8},
    v::Matrix{UInt8},
    syndrome_based::Bool,
    check_adj_list::Vector{Vector{Int}},
    ...
    @inbounds while iter < max_iter
        ...
        if syndrome_based
            all(syn[i] % 2 == v[i, 1] for i = 1:num_check) &&
                return true, current_bits, iter, var_to_check_messages[:, curr_iter]
        else
            all(iszero(syn[i] % 2) for i = 1:num_check) &&
                return true, current_bits, iter, var_to_check_messages[:, curr_iter]
        end
...
function _message_passing_fast_layered(
    H_Int::Matrix{UInt8},
    v::Matrix{UInt8},
    syndrome_based::Bool,
    check_adj_list::Vector{Vector{Int}},
    ...
    @inbounds while iter < max_iter
        for layer in layers
            for c in layer
                ...
                for v in check_adj_list[c]
                    S += phi(
                        var_to_check_messages[v, prev_iter] - check_to_var_messages[c][v],
                    )
                end
                ...
                for v in check_adj_list[c]
                    ...
                    var_to_check_messages[v, curr_iter] =
                        Q_temp + check_to_var_messages[c][v]
                end
            end
            ...
            @inbounds @simd for c in layer
                for v in check_adj_list[c]
                    var_to_check_messages[v, prev_iter] =
                        var_to_check_messages[v, curr_iter]
                end
            end
        end
...
function _message_passing_Int(
    H::Matrix{T},
    syndrome::Union{Missing,Vector{T}},
    chn_inits::Vector{Int},
    c_to_v_mess::Function,
    var_adj_list::Vector{Vector{Int}},
    check_adj_list::Vector{Vector{Int}},
    max_iter::Int,
    ...
    @simd for c = 1:num_check
        for v in check_adj_list[c]
            if !ismissing(syndrome)
                check_to_var_messages[c, v, 1] =
                    (-1)^syndrome[c] *
                    c_to_v_mess(c, v, 1, check_adj_list, var_to_check_messages, 0.0)
            else
                check_to_var_messages[c, v, 1] =
                    c_to_v_mess(c, v, 1, check_adj_list, var_to_check_messages, 0.0)
            end
    ...
    LinearAlgebra.mul!(syn, H, current_bits)
    if !ismissing(syndrome)
        all(syn[i] .% 2 == syndrome[i] for i = 1:num_check) && return true, current_bits, 1
    else
        all(iszero(syn[i] .% 2) for i = 1:num_check) && return true, current_bits, 1
    end
    ...
    @inbounds while iter ≤ max_iter
        @simd for v = 1:num_var
            for c in var_adj_list[v]
                if kind == :A && length(var_adj_list[v]) > 1
                    if all(
                        !Base.isequal(chn_inits[v]),
                        check_to_var_messages[c2, v, prev_iter] for
        ...
        @simd for c = 1:num_check
            for v in check_adj_list[c]
                if !ismissing(syndrome)
                    ...
                    check_to_var_messages[c, v, 1] = c_to_v_mess(
                        c,
                        v,
                        curr_iter,
                        check_adj_list,
        ...
        @simd for v = 1:num_var
            len = length(var_adj_list[v])
            one_count =
                count(isone, view(check_to_var_messages, var_adj_list[v], v, curr_iter))
            d = fld(len, 2)
            current_bits[v] = one_count + (isone(chn_inits[v]) && iseven(len)) > d
        end
        ...
        if !ismissing(syndrome)
            all(syn[i] .% 2 == syndrome[i] for i = 1:num_check) &&
                return true, current_bits, curr_iter
        else
            all(iszero(syn[i] .% 2) for i = 1:num_check) &&
                return true, current_bits, curr_iter
        end
...
function _message_passing_decimation(
    H::Matrix{T},
    w::Vector{T},
    chn_inits::Union{Missing,Vector{Float64}},
    c_to_v_mess::Function,
    ...
    kind::Symbol,
    schedule::Symbol,
    decimated_bits::Vector{Int},
    decimated_values::Vector{Int},
    current_bits::Vector{Int},
    totals::Union{Vector{Int},Vector{Float64}},
    syn::Vector{Int},
    check_to_var_messages::Union{Array{Float64,3},Array{Int,3}},
    var_to_check_messages::Union{Array{Float64,3},Array{Int,3}},
    Bt::Int,
    attenuation::Float64,
    algorithm::Symbol,
    guided_rounds::Int,
        ...
        elseif kind ∈ (:A, :B)
            @simd for v = 1:num_var
                if v ∉ decimated_bits
                    ...
                    one_count = count(
                        isone,
                        view(check_to_var_messages, var_adj_list[v], v, curr_iter),
                    )
                    d = fld(len, 2)
...
function layered_schedule(
    ...
    elseif schedule == :parallel
        ...
        # serial
        sched_list = [[i] for i = 1:num_check]
        random && shuffle!(sched_list)
    end
    return sched_list
end
...
function balance_of_layered_schedule(sch::Vector{Vector{Int}})
    ...
    any(x -> is_empty(x), sch) &&
        throw(ArgumentError("Schedule cannot contain an empty layer"))
