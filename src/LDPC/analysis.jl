...
function _find_lambda_given_rho(ρ::Union{Vector{Float64}, PolyRingElem}, ε::Float64, l_max::Int; Δ = 0.001)
    model = Model(GLPK.Optimizer)
    @variable(model, λ[1:l_max - 1] >= 0)
    @constraint(model, sum(λ) == 1)
    for x in 0:Δ:1
        @constraint(model, ε * sum(λ[i] * (1 - _poly_eval(1 - x, ρ))^i for i in 1:l_max - 1) - x <= 0)
    end
    @constraint(model, ε * _d_poly_eval(1, ρ) * λ[1] <= 1)
...
function _find_rho_given_lambda(λ::Union{Vector{Float64}, PolyRingElem}, ε::Float64, r_max::Int; Δ = 0.001)
    model = Model(GLPK.Optimizer)
    @variable(model, ρ[1:r_max - 1] >= 0)
    @constraint(model, sum(ρ) == 1)
    for x in 0:Δ:1
        @constraint(model, 1 - x - sum(ρ[i] * (1 - ε * _poly_eval(x, λ))^i for i in 1:r_max - 1) <= 0)
    end
...
"""
function optimal_lambda(ρ, l_max::Int, param::Float64, var_type::Symbol; Δ = 0.001)
    var_type ∈ (:r, :ε) || throw(ArgumentError("var_type must be :r for target rate or :ε for threshold"))
    var_type == :r && param >= 1 - 2 * _integrate_poly_0_1(ρ) && throw(ArgumentError("This rate is unachieveable with the given ρ."))
...
"""
function optimal_rho(λ, r_max::Int, param::Float64, var_type::Symbol, Δ = 1e-3)
    var_type ∈ (:r, :ε) || throw(ArgumentError("var_type must be :r for target rate or :ε for threshold"))
    var_type == :r && param >= 1 - 1 / (r_max * _integrate_poly_0_1(λ)) && throw(ArgumentError("This rate is unachieveable with the given r_max and λ."))
    # TODO: check for when var_type == :ε as well
...
function _optimal_distributions(poly, poly_type::Symbol, var_max::Int, real_param::Float64, var_type::Symbol; Δλ = 0.001, Δρ = 0.001)
    int_poly = _integrate_poly_0_1(poly)
    if var_type == :r
        tolerance = 1e-6
        max_iters = 100
        high = 1.0
        ...
        while !(0 <= Δ <= tolerance) && count < max_iters
            ...
            Δ, sol = try
                if poly_type == :ρ
                    ...
                    sol_rate = 1 - int_poly / _integrate_poly_0_1(sol)
                else
                    sol, _ = _find_rho_given_lambda(poly, mid, var_max, Δ = Δρ)
                    sol_rate = 1 - _integrate_poly_0_1(sol) / int_poly
                end
...
function _find_lambda_and_rho(l_max::Int, r_max::Int, ε::Float64; Δρ = 0.01, Δλ = 0.001)
    ...
    for i in 1:iters
        ...
        try
            ...
            rates[i] = 1 - _integrate_poly_0_1(ρ[:, i]) / _integrate_poly_0_1(λ[:, i])
        catch end
    end
    i = argmax(rates)
...
"""
function optimal_lambda_and_rho(l_max::Int, r_max::Int, param::Float64, var_type::Symbol; Δρ = 0.01, Δλ = 0.001)
    if var_type == :r
        tolerance = 1e-6
        max_iters = 100
        high = 1.0
