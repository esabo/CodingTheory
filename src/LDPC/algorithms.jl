...
Return a parity-check matrix built with the progressive-edge-growth (PEG)
algorithm and approximate-cycle-EMD (ACE) maximization.
...
Return a quasi-cyclic shift matrix built with the QC-PEG algorithm to increase
the lifted graph's girth while preserving a block-circulant structure.
...
# Returns
* A matrix of the same size as `B` containing the selected circulant shifts
  `p ∈ [0, Z-1]`.
...
Return a parity-check matrix lifted from a protograph base matrix with the PEG
algorithm, which selects permutation edges to increase girth.
...
Return a random LDPC code generated by the MacKay-Neal edge-placement
algorithm.

The code has `n` variable nodes, `m` check nodes, and variable-node degrees
given by `deg_v`. Edge placement balances check degrees while excluding parallel
edges and 4-cycles. At most `max_retries` attempts are made for each column.
"""
...
The equally sized component matrices are placed along a diagonal coupling
window for `L` variable-block positions. If there are ``w + 1`` components of
size ``m_b \\times n_b``, the parity-check matrix has size
``(L + w)m_b \\times Ln_b``.
"""
...
Return the Euclidean-geometry LDPC code from point-line incidence in
``\\operatorname{EG}(m, p)``.

Here `m` is the dimension, and `p` is the prime field order expected by the
construction. Columns of the parity-check matrix represent points and rows
represent affine lines.
"""
...
Return the projective-geometry LDPC code from point-line incidence in
``\\operatorname{PG}(m, p)``.

Here `m` is the projective dimension, and `p` is the prime field order expected
by the construction. Columns of the parity-check matrix represent projective
points and rows represent projective lines.
"""
...
Return a random regular LDPC code from Gallager's block-permutation
construction.

For the regular construction, `wr` must divide `n`. The parity-check matrix then
has `n` columns, column weight `wc`, and row weight `wr`. Its first row block has
disjoint consecutive supports, and each remaining block is an independent
random column permutation of that block.
"""
