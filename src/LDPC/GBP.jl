...
The variable-node labels are stored in `id`. The integer vectors `parents`,
`ancestors`, and `subregions` index regions in the containing `RegionGraph`;
`parents` and `ancestors` point toward strict supersets, while `subregions`
points toward strict subsets. The Möbius counting number is stored in
`overcounting_number`.
"""
struct Region
...
For a graph built by `region_graph_from_base_nodes`, this number is one minus
the sum of the counting numbers of all ancestors of `r`.
"""
...
Return the region graph generated by closing the row supports of `H` under
nonempty proper intersections.

The row supports are the base regions. The overload for an LDPC code uses its
parity-check matrix.
"""
...
The supplied sets are the base regions. Additional regions are their nonempty
proper intersections, with parent, ancestor, descendant, and Möbius counting
data populated in the resulting graph.
"""
...
For every variable, the counting numbers of regions containing it must sum to
one. For every region, its counting number plus those of all its ancestors must
also sum to one.
"""
...
Return a region graph obtained by removing removable regions whose counting
number is zero and compacting the stored region indices.
...
Return a region graph with redundant parent edges to non-immediate ancestors
removed.

The ancestor relation and counting numbers are preserved, and `R` itself is
returned when no such edge exists.
"""
...
Return the maximal cliques produced by min-degree elimination of the primal
graph of `H`.

Variable nodes are adjacent when they occur in a common row of `H`; fill edges
added during elimination triangulate this graph. Each clique is returned as a
`BitSet` of variable indices.
"""
...
The workspace stores region-local factors and beliefs, parent-to-child messages,
marginalization maps, update order, variable incidence data, and reusable
decoding buffers.
"""
struct GBPWorkspace
    num_regions::Int
...
Return a `GBPWorkspace` for the parent-to-child edges of `R` and the dimensions
of parity-check matrix `H`.

The workspace allocates state tables for each region and message, constructs
parent-to-child marginalization maps, and prepares a two-way topological update
schedule.
"""
...
Return `(success, hard_decisions, iterations)`.
"""
