...
function McEliece_attack(G::CTMatrix, w::CTMatrix, t::Int; max_iters::Int = 10000, verbose::Bool = false)
    ...
    answers = [zeros(Int, 1, n) for _ in 1:num_thrds]
    which_ans = [false for _ in 1:num_thrds]
    perms = [collect(1:n) for _ in 1:num_thrds]
    ...
    Threads.@threads for th in 1:num_thrds
        ...
        for _ in 1:(thread_load + (th <= remaining ? 1 : 0))
            if flag[]
                wt = 0
...
function Lee_Brickell_attack(G::CTMatrix, w::CTMatrix, t::Int, p::Int; max_iters::Int = 10000, verbose::Bool = false)
    # check 0 matrix, 0 dimensions
...
function Leon_attack(G::CTMatrix, w::CTMatrix, t::Int, p::Int, l::Int; max_iters::Int = 10000, verbose::Bool = false)
...
function generalized_Stern_attack(G::CTMatrix, w::CTMatrix, t::Int, p::Int, l::Int, s::Int; max_iters::Int = 10000, verbose::Bool = false)
    ...
    Threads.@threads for th in 1:num_thrds
        ...
        Xis = [Dict{Matrix{UInt8}, Vector{Matrix{UInt8}}}() for _ in 1:s - 1]
        ...
        for _ in 1:(thread_load + (th <= remaining ? 1 : 0))
            if flag[]
                ...
                for m in GrayCode(nr_Xi, p)
                    for i in 1:s - 1
                        LinearAlgebra.mul!(q_i, m, view(G_loc, (i - 1) * nr_Xi + 1:i * nr_Xi, k + 1:k + l))
                        q_i .+= w_loc[1, k + 1:k + l]
                        if haskey(Xis[i], q_i)
                            push(Xis[i][q_i], m)
                        else
                ...
                keys_Xis = [keys(Xi_s) for _ in 1:length(Xis)]
                vals_Xis = [values(Xi_s) for _ in 1:length(Xis)]
                for m in GrayCode(nr_Y, p)
                    if flag[]
                        # q_i now representing q_Y
                        LinearAlgebra.mul!(q_i, m, view(G_loc, k - nr_Y + 1:k, k + 1:k 
                        + l))
                        q_i .+= w_loc[1, k + 1:k + l]
                        for iter_q in Nemo.AbstractAlgebra.ProductIterator([1:length(Xis) for _ in 1:s - 1], inplace = true)
                            if flag[]
                                q_sum .= keys_Xis[1][iter_q[1]]
                                for i in 2:length(iter_q)
                                    q_sum .+= keys_Xis[i][iter_q[i]]
                                end

                                if q_sum == q_i
                                    for iter_m in Nemo.AbstractAlgebra.ProductIterator([1:length(vals_Xis[i][iter_q[i]] for i in 1:length(Xis))])
                                        # make m_B, q_B
                                        for i in 1:s - 1
                                            m_B[1, (i - i) * nr_Xi + 1:i * nr_Xi] .= vals_Xis[i][iter[i]][iter_m[i]]
                                        end
                                        LinearAlgebra.mul!(q_B, m_B, view(G_loc, :, k + l + 1:n))
                                        q_B .+= w_loc[1, k + l + 1:n]
                                        wt_B = 0
                                        @inbounds for j in axes(q_B, 2)
                                            isodd(q_B[1, j]) && (wt_B += 1;)
...
Stern_attack(G::CTMatrix, w::CTMatrix, t::Int, p::Int, l::Int, s::Int; max_iters::Int = 10000, verbose::Bool = false) = generalized_Stern_attack(G, w, t, p, l, 1, max_iters = max_iters, verbose = verbose)
