...
Return the linear code defined by generator matrix `G` and parity-check matrix
`H`, including when the matrices mix sparse and dense representations.
"""
...
Return the linear code generated by an integer matrix over the finite field of
order `q`.
"""
...
Return the linear code generated by integer row vectors over the finite field
of order `q`.
"""
...
alias for `⊆`; use `⊊` to test proper containment.
"""
...
# Notes
- Containment is proper exactly when it holds and the dimensions differ, since a
  subcode of the same dimension is the whole code.
"""
⊊(C1::AbstractLinearCode, C2::AbstractLinearCode) = C1.k < C2.k && C1 ⊆ C2
...
function l_Galois_hull(C::AbstractLinearCode, l::Int)
    D = l_Galois_dual(C, l)
    G = generator_matrix(C)
    # the generators of D span the Galois dual; its parity-check matrix would
    # dualize a second time
    ...
    dim_I = AbstractAlgebra.dim(I)
    if !iszero(dim_I)
        I_basis = [U_to_VS(I_to_W(g)) for g in gens(I)]
        G_hull = zero_matrix(F, dim_I, C.n)
        for (j, v) in enumerate(I_basis), i in 1:C.n
            G_hull[j, i] = v[i]
        end
        return LinearCode(G_hull), dim_I
    else
        return missing, 0 
    end
end
...
# Notes
- The product runs over the nonzero weights of the dual code, so this requires
  the weight distribution of ``C^\\perp``.
"""
function characteristic_polynomial(C::AbstractLinearCode)
    _, x = polynomial_ring(Nemo.QQ, :x)
    D = dual(C)
    sup_D = _weight_support(D)
    q = Int(order(C.F))
    return q^(C.n - C.k) * prod(1 - x // j for j in sup_D if j > 0)
end
...
Return all codewords of `C`, or print them and return `nothing` when
`only_print` is `true`. This is an alias for `words`; enumerating all
``q^k`` codewords can require substantial time and memory.
"""
...
Return all elements of `C`, or print them and return `nothing` when
`only_print` is `true`. This is an alias for `codewords` and `words`;
enumerating all ``q^k`` elements can require substantial time and memory.
"""
...
Return the Euclidean hull ``C \\cap C^\\perp`` and its dimension.
This is an alias for `hull`; the code component is `missing` when the
hull has dimension zero.
"""
...
function Hermitian_hull(C::AbstractLinearCode)
    D = Hermitian_dual(C)
    G = generator_matrix(C) # Safe getter
    # the generators of D span the Hermitian dual; its parity-check matrix would
    # dualize a second time
    H = generator_matrix(D) # Safe getter
    F = field(C)
    VS = AbstractAlgebra.vector_space(F, C.n)
