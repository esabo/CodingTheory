...
Return the matrix whose entries are the numbers of nonzero coefficients in
the polynomial representatives of the entries of `A`.
"""
...
Return the base matrix whose entries are the polynomial weights of `A`.
This is an alias for `weight_matrix`.
"""
...
Return the protograph matrix whose entries are the polynomial weights of `A`.
This is an alias for `weight_matrix` and `base_matrix`.
"""
...
Return the exponent matrix of `C`, using `-1` for zero circulants.
This is an alias for `shift_matrix` and requires every nonzero polynomial
entry to be a monomial.
"""
