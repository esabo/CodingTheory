...
Return the permutation generator for the cyclic group of order n.
"""
...
Return the permutation generators for a 2D cyclic code of size n1 × n2.
...
Return the two generators for PSL(2, p) which form the automorphism group 
...
Return the generators for the General Affine Group GA(m, 2) acting on a 
...
Return the systematic matrices, associated column permutations, and ranks
produced from `G` by the selected information-set algorithm. When `only_A`
is `true`, return only each nonsystematic block in the matrix collection;
when `permute` is `true`, move each selected information set to the front.
"""
...
Return `(gen_mats, perms, rnks)` after partitioning the coordinates
``\\{1, \\dots, n\\}`` into disjoint systematic sets ``T_1, \\dots, T_s``, where
`perms` are the disjoint column sets.
"""
...
Return true if the codeword 'v' has not been generated in a previous 
...
Return the absolute minimum parity weight generated by any pair of remaining 
message symbols over ``\\mathrm{GF}(q)``. This is the non-binary generalization of the 
"Brouwer Trick", allowing the recursive search engine to replace deep tree branching 
with an ``O(1)`` table lookup for the final two symbols.

# Mathematical Optimization
Instead of naively checking all ``(q-1)^2`` possible non-zero combinations of two 
message symbols ``m_i c_i + m_j c_j``, this function exploits the fact that 
Hamming weight is invariant under scalar multiplication. It calculates the minimum 
weight of ``c_i + \\gamma c_j`` for all non-zero scalars ``\\gamma``, reducing the 
...
Return `(H_packed, L_packed)`, the compression of a ``\\mathrm{GF}(3)`` matrix into
orthogonal bitsliced `UInt64` matrices.
"""
...
Return the ``\\mathrm{GF}(4)`` Brouwer weight-2 table computed directly from the
bitsliced matrices, evaluating ``c_i + \\gamma c_j`` for
``\\gamma \\in \\{1, \\omega, \\omega^2\\}``.
"""
...
Return a systematic codeword mapped back to the original coordinates.
- `msg_bits`: the weight-`r` message, the first `k` coordinates of the systematic generator matrix
- `packed_tail`: the XORed parity bits, the remaining `n - k` coordinates
- `perm`: the permutation `[info_indices; parity_indices]` for this set
"""
...
Return `(d, witness)` with `witness::Vector{Int}`. If no word of weight at
...
Return the generator matrix of the subcode, or nothing if trivial.
"""
...
Return the updated global minimum codeword if a smaller weight is found.
"""
