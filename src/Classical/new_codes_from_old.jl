...
function u_u_plus_v(C1::AbstractLinearCode, C2::AbstractLinearCode)
    C1.F == C2.F || throw(
        ArgumentError("Base field must be the same in the Plotkin (u|u + v)-construction."),
    )
    C1.n == C2.n || throw(
        ArgumentError(
            "Both codes must have the same length in the Plotkin (u|u + v)-construction.",
        ),
    )
    (iszero(C1.G) || iszero(C2.G)) &&
        throw(ArgumentError("`u_u_plus_v` not supported for zero codes."))
...
function u_plus_w_v_plus_w_u_plus_v_plus_w(C1::AbstractLinearCode, C2::AbstractLinearCode)
    C1.F == C2.F || throw(
        ArgumentError(
            "All codes must be over the same base ring in the (u + w | v + w | u + v + w)-construction.",
        ),
    )
    C1.n == C2.n || throw(
        ArgumentError(
            "All codes must be the same length in the (u + w | v + w | u + v + w)-construction.",
        ),
    )
    ...
    return LinearCode(vcat(hcat(G1, Z, G1), hcat(G2, G2, G2), hcat(Z, G1, G1)))
end
...
"""
function construction_X(
    C1::AbstractLinearCode,
    C2::AbstractLinearCode,
    C3::AbstractLinearCode,
)
    C1 ⊆ C2 || throw(
        ArgumentError("The first code must be a subcode of the second in construction X."),
    )
    are_equivalent(C1, C2) && throw(
        ArgumentError(
            "The first code must be a proper subcode of the second in construction X.",
        ),
    )
    C1.F == C2.F == C3.F ||
        throw(ArgumentError("All codes must be over the same base ring in construction X."))
    C2.k == C1.k + dimension(C3) || throw(
        ArgumentError(
            "The dimension of the second code must be the sum of the dimensions of the first and third codes.",
        ),
    )
    ...
    C = LinearCode(
        vcat(
            hcat(generator_matrix(C1 / C2), generator_matrix(C3)),
...
function construction_X3(
    ...
    C1 ⊆ C2 || throw(
        ArgumentError("The first code must be a subcode of the second in construction X3."),
    )
    are_equivalent(C1, C2) && throw(
        ArgumentError(
            "The first code must be a proper subcode of the second in construction X3.",
        ),
    )
    C2 ⊆ C3 || throw(
        ArgumentError("The second code must be a subcode of the third in construction X3."),
    )
    are_equivalent(C2, C3) && throw(
        ArgumentError(
            "The second code must be a proper subcode of the third in construction X3.",
        ),
    )
    # the above lines check C1.F == C2.F == field(C3)
    C1.F == C4.F == C5.F || throw(
        ArgumentError("All codes must be over the same base ring in construction X3."),
    )
    C3.k == C2.k + C4.k || throw(
        ArgumentError(
            "The dimension of the third code must be the sum of the dimensions of the second and fourth codes in construction X3.",
        ),
    )
    C2.k == C1.k + C5.k || throw(
        ArgumentError(
            "The dimension of the second code must be the sum of the dimensions of the first and fifth codes in construction X3.",
        ),
    )
    ...
    # could do some verification steps on parameters
    G = vcat(
        hcat(generator_matrix(C1), zero_matrix(F, C1.k, C4.n), zero_matrix(F, C1.k, C5.n)),
...
function ×(C1::AbstractLinearCode, C2::AbstractLinearCode)
    ...
    if !ismissing(C1.d) && !ismissing(C2.d)
        ...
        return LinearCode(
            ...
            k,
            missing,
            C1.l_bound * C2.l_bound,
            C1.u_bound * C2.u_bound,
            G,
            H,
            G_stand,
            H_stand,
...
"""
⊗(C1::AbstractLinearCode, C2::AbstractLinearCode) =
    LinearCode(parity_check_matrix(C1) ⊗ parity_check_matrix(C2), true)
...
function entrywise_product_code(C::AbstractLinearCode, D::AbstractLinearCode)
    C.F == D.F ||
        throw(ArgumentError("Codes must be over the same field in the Schur product."))
    C.n == D.n ||
        throw(ArgumentError("Codes must have the same length in the Schur product."))
    if isa(C, ReedMullerCode)
        ...
        return LinearCode(
            matrix(C.F, reduce(vcat, G_C[i, :] .* G_D[j, :] for (i, j) in indices)),
        )
    end
    # TODO: verify C ⊂ it?
end
...
function /(C2::AbstractLinearCode, C1::AbstractLinearCode)
    ...
    U, U_to_V = sub(V, [V(G1[i, :]) for i = 1:nrows(G1)])
    W, W_to_V = sub(V, [V(G2[i, :]) for i = 1:nrows(G2)])
    ...
    F_basis = [
        [F(C2_mod_C1_basis[j][i]) for i = 1:dim(parent(C2_mod_C1_basis[1]))] for
        j = 1:length(C2_mod_C1_basis)
    ]
...
function juxtaposition(C1::AbstractLinearCode, C2::AbstractLinearCode)
    C1.F == C2.F ||
        throw(ArgumentError("Cannot juxtapose two codes over different fields."))
    G1 = generator_matrix(C1)
    G2 = generator_matrix(C2)
    nrows(G1) == nrows(G2) || throw(
        ArgumentError(
            "Cannot juxtapose codes with generator matrices with a different number of rows.",
        ),
    )
...
function permute_code(C::AbstractLinearCode, σ::Union{PermGroupElem,Perm{Int},Vector{Int}})
    if isa(C, QuasiCyclicCode)
        ...
        size(P, 1) == C.n ||
            throw(ArgumentError("Incorrect number of digits in permutation."))
...
function extend(C::AbstractLinearCode, a::CTMatrixTypes, c::Integer)
    1 <= c <= C.n + 1 || throw(
        ArgumentError(
            "The code has length $(C.n), so `c` must be between 1 and $(C.n + 1).",
        ),
    )
    ...
    H_new = vcat(
        new_row,
        hcat(view(C.H, :, 1:(c-1)), zero_matrix(F, nrows(C.H), 1), view(C.H, :, c:C.n)),
    )
...
extend(C::AbstractLinearCode, c::Integer) =
    extend(C, matrix(base_ring(C.G), 1, C.n, ones(Int, C.n)), c)
extend(C::AbstractLinearCode, a::CTMatrixTypes) = extend(C, a, C.n + 1)
extend(C::AbstractLinearCode) =
    extend(C, matrix(base_ring(C.G), 1, C.n, ones(Int, C.n)), C.n + 1)
...
function puncture(C::AbstractLinearCode, cols::Vector{<:Integer})
    ...
    cols ⊆ 1:C.n ||
        throw(ArgumentError("Columns to puncture are not a subset of the index set."))
    length(cols) == C.n &&
        throw(ArgumentError("Cannot puncture all columns of a generator matrix."))
...
function expurgate(C::AbstractLinearCode, rows::Vector{<:Integer})
    ...
    nr = nrows(G)
    rows ⊆ 1:nr ||
        throw(ArgumentError("Rows to expurgate are not a subset of the index set."))
    length(rows) == nr &&
        throw(ArgumentError("Cannot expurgate all rows of a generator matrix."))
...
"""
shorten(C::AbstractLinearCode, L::Vector{<:Integer}) =
    isempty(L) ? (return C;) : (return dual(puncture(dual(C), L));)
...
function augment(C::AbstractLinearCode, M::CTMatrixTypes)
    ...
    C.n == ncols(M) || throw(
        ArgumentError(
            "Rows to augment must have the same number of columns as the generator matrix.",
        ),
    )
    C.F == base_ring(M) ||
        throw(ArgumentError("Rows to augment must have the same base field as the code."))
...
function subcode(C::AbstractLinearCode, k::Int)
    k >= 1 && k < C.k || throw(
        ArgumentError(
            "Cannot construct a $k-dimensional subcode of an $(C.k)-dimensional code.",
        ),
    )
...
"""
function subcode_of_dimension_between_codes(
    C1::AbstractLinearCode,
    C2::AbstractLinearCode,
    k::Int,
)
    ...
    C2.k <= k <= C1.k ||
        throw(ArgumentError("The dimension must be between that of C1 and C2."))
...
"""
function subfield_subcode(
    C::AbstractLinearCode,
    K::CTFieldTypes,
    basis::Vector{<:CTFieldElem},
)
    C_new = LinearCode(
        transpose(expand_matrix(transpose(parity_check_matrix(C)), K, basis)),
        true,
    )
    C_new.G = change_base_ring(K, C_new.G)
    C_new.H = change_base_ring(K, C_new.H)
...
"""
trace_code(C::AbstractLinearCode, K::CTFieldTypes, basis::Vector{<:CTFieldElem}) =
    dual(subfield_subcode(dual(C), K, basis))
...
function even_subcode(C::AbstractLinearCode)
    ...
    homo1_quad = ModuleHomomorphism(
        V_C,
        G_F_VS,
        matrix(F, dim(V_C), 1, reduce(vcat, [F(weight(ψ(g).v) % 2) for g in gens(V_C)])),
    )
...
function doubly_even_subcode(C::AbstractLinearCode)
    ...
    if !iszero(dim(even_sub))
                        ...
                        F(
                            weight(
                                matrix(
                                    ...
                                    1,
                                    C.n,
                                    ψ(ϕ1(gens(even_sub)[i])).v .*
                                    ψ(ϕ1(gens(even_sub)[j])).v,
                                ),
                            ) % 2,
                        ) for i = 1:dim(even_sub), j = 1:dim(even_sub)
                    ],
                ),
            ),
        )
        ...
        if !iszero(dim(even_sub_w_overlap))
            ...
            homo2_quad = ModuleHomomorphism(
                even_sub_w_overlap,
                G_F_VS,
                matrix(
                    F,
                    dim(even_sub_w_overlap),
                    1,
                    reduce(
                        vcat,
                        [
                            F(div(weight(ψ(ϕ1(μ1(g))).v), 2) % 2) for
                            g in gens(even_sub_w_overlap)
                        ],
                    ),
                ),
            )
            ...
            if !iszero(dim(four_sub))
                return LinearCode(
                    reduce(vcat, [ψ(ϕ1(μ1(ϕ2(g)))).v for g in gens(four_sub)]),
                )
            end
        end
    end
