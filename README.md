...
The quantum part of this library is designed to eventually replace the good package in MAGMA, which is not free. At the same time, the MAGMA library is limited in scope and the features of this library already surpass it. Work still needs to be done to extend the quantum code to include more useful features to the community.
