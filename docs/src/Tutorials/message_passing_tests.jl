function CSS_decoder_simple(S::AbstractStabilizerCode)
    ...
    for i = 1:len_noise
        ...
        Threads.@threads for th = 1:num_threads
            ...
            totals,
            X_syn,
            Z_syn = CodingTheory._message_passing_init_CSS(
                S,
                chn,
                max_iter,
                missing,
            ...
            for _ = 1:runs_per_thread
                ...
                # decode X
                X_flag, X_out, X_iter = CodingTheory._message_passing_layered(
                    X_stabs,
                    X_meas_syn,
                    X_chn_inits,
                    CodingTheory._SP_check_node_message_box_plus,
                    X_var_adj_list,
                    X_check_adj_list,
                    max_iter,
                    schedule,
                ...
                # if bypass
                Z_flag, Z_out, Z_iter = CodingTheory._message_passing_layered(
                    Z_stabs,
                    Z_meas_syn,
                    Z_chn_inits,
                    CodingTheory._SP_check_node_message_box_plus,
        ...
        @inbounds FER[i] =
            (sum(local_conv_failure_counts) + sum(local_log_failure_counts)) / new_num_runs
        println("FER = $(FER[i])")
    ...
    @inbounds for i = 2:num_threads
        @simd for j = 1:max_iter
...
function _make_single_shot_Tanner_graph(H::T, M::T) where {T<:CodingTheory.CTMatrixTypes}
    ...
    row = 1
    @inbounds @simd for c = (nc_H+1):(nc_H+nc_M)
...
function CSS_metachecks(
    X_stabs::T,
    X_logs::T,
    X_meta::T,
    X_meta_L::T,
) where {T<:CodingTheory.CTMatrixTypes}
    nr, n = size(X_stabs)
    ...
    iszero(X_meta * X_stabs) ||
        throw(ArgumentError("Either metachecks or stabilizers wrong"))
    X_ss = _make_single_shot_Tanner_graph(X_stabs, X_meta)

    # noise details
    p_meas = 1e-3
    chn_meas = MPNoiseModel(:BSC, p_meas)
    chn_inits_meta_in = Float64[log((1 - p_meas) / p_meas) for _ = 1:nr]
    ...
    local_log_failure_counts_scheme_2 = [zeros(Int, nr_logs) for _ = 1:num_threads]
    ...
    for i = 1:len_noise
        ...
        chn_inits_stabs_in = Float64[log((1 - p) / p) for _ = 1:n]
        ...
        Threads.@threads for th = 1:num_threads
            ...
            _,
            var_adj_list_stabs,
            check_adj_list_stabs,
            chn_inits_stabs,
            check_to_var_messages_stabs,
            var_to_check_messages_stabs,
            current_bits_stabs,
            totals_stabs,
            syn_stabs = CodingTheory._message_passing_init(
                X_stabs,
                v,
                chn,
                max_iter,
            ...
            check_adj_list_meta,
            chn_inits_meta,
            check_to_var_messages_meta,
            var_to_check_messages_meta,
            current_bits_meta,
            totals_meta,
            syn_meta = CodingTheory._message_passing_init(
                X_meta,
                v2,
                chn_meas,
                max_iter,
            ...
            _,
            var_adj_list_meta_L,
            check_adj_list_meta_L,
            chn_inits_meta_L,
            check_to_var_messages_meta_L,
            var_to_check_messages_meta_L,
            current_bits_meta_L,
            totals_meta_L,
            syn_meta_L = CodingTheory._message_passing_init(
                X_meta_L,
                v2,
                chn_meas,
                max_iter,
            ...
            check_adj_list_ss,
            chn_inits_ss,
            check_to_var_messages_ss,
            var_to_check_messages_ss,
            current_bits_ss,
            totals_ss,
            syn_ss = CodingTheory._message_passing_init(
                X_ss,
                v3,
                chn,
                max_iter,
            ...
            for _ = 1:runs_per_thread
                ...
                X_out_stabs = Int[0]
                X_out_ss = Int[0]
                for ss_iter = 1:single_shot_rounds
                    # sample
                    sample!(Pauli_types, Pauli_weights, Z_err)
                    @inbounds @simd for j = i:n
                    ...
                    if ss_iter != single_shot_rounds
                        ...
                        @inbounds @simd for j = 1:nr
                            X_meas_syn_scheme_1[j] =
                                (X_meas_syn_scheme_1[j] + X_syn_err[j]) % 2
                            X_meas_syn_scheme_2[j] =
                                (X_meas_syn_scheme_2[j] + X_syn_err[j]) % 2
                        end
                    end
                    ...
                    @inbounds @simd for j = 1:length(m)
                        m[j] %= 2
                        m_2[j] %= 2
                    end
                    ...
                    if !iszero(m)
                        ...
                        X_flag_meta, X_out_meta, _, = CodingTheory._message_passing_layered(
                            X_meta_Int,
                            X_meas_syn_scheme_1,
                            chn_inits_meta,
                            CodingTheory._SP_check_node_message_box_plus,
                        ...
                        if X_flag_meta
                            ...
                            @inbounds @simd for j = 1:length(m2)
                                m2[j] %= 2
                            end
                            ...
                            if !iszero(m2)
                                ...
                                X_flag_meta_L, X_out_meta_L, _, =
                                    CodingTheory._message_passing_layered(
                                        X_meta_L_Int,
                                        X_meas_syn_scheme_1,
                                        chn_inits_meta_L,
                                        CodingTheory._SP_check_node_message_box_plus,
                    ...
                    # decode stabilizers
                    X_flag_stabs, X_out_stabs, _, = CodingTheory._message_passing_layered(
                        X_stabs_Int,
                        X_meas_syn_scheme_1,
                        chn_inits_stabs,
                        CodingTheory._SP_check_node_message_box_plus,
                    ...
                    X_flag_ss, X_out_ss, _, = CodingTheory._message_passing_layered(
                        X_ss_Int,
                        Int[X_meas_syn_scheme_2; m_2],
                        chn_inits_ss,
                        CodingTheory._SP_check_node_message_box_plus,
                        var_adj_list_ss,
                        check_adj_list_ss,
        ...
        # reduce iteration counts
        @inbounds for i = 2:num_threads
            @simd for j = 1:nr_logs
                local_log_failure_counts_scheme_1[1][j] +=
                    local_log_failure_counts_scheme_1[i][j]
                local_log_failure_counts_scheme_2[1][j] +=
                    local_log_failure_counts_scheme_2[i][j]
            end
        end
